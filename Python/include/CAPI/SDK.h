...
typedef struct GENICAM_Frame
{
	...
	/// \~chinese
	/// \brief 获取图片时间戳(仅对GigE/Usb/PCIe相机有效)
	/// \param [in] thiz 本接口指针
	/// \return 返回图片时间戳，失败时为0
	/// \~english
	/// \brief get time stamp of image(GigE/Usb/PCIe Camera Only)
	/// \param [in] thiz this pointer
	/// return image time stamp, return 0 when failed
	uint64_t (*getImageTimeStamp)(struct GENICAM_Frame *thiz);

	/// \~chinese
	/// \brief 获取图片对应的BlockId(仅对GigE/Usb/PCIe相机有效)
	/// \param [in] thiz 本接口指针
	/// \return 返回图片对应的BlockId，失败时为0
	/// \~english
	/// \brief get the image's block ID(GigE/Usb/PCIe Camera Only)
	/// \param [in] thiz this pointer
	...
	/// \~chinese
	/// \brief 获取Buffer中包含的Chunk个数(仅对GigE/Usb相机有效)
	/// \param [in] thiz 本接口指针
	/// \return Chunk个数
	/// \~english
	/// \brief get the number of chunk in buffer(GigE/Usb Camera Only)
	/// \param [in] thiz this pointer
	/// \return the number of chunk
	uint32_t (*getChunkCount)(struct GENICAM_Frame *thiz);

	/// \~chinese
	/// \brief 根据索引获取Chunk数据(仅对GigE/Usb相机有效)
	/// \param [in] thiz 本接口指针
	...
	/// \retval 0   获取成功
	/// \~english
	/// \brief get chunk data by index(GigE/Usb Camera Only)
	/// \param [in] thiz this pointer
	...
	/// \~chinese
	/// \brief 获取图片paddingX(仅对GigE/Usb/PCIe相机有效)
	/// \param [in] thiz 本接口指针
	/// \return 返回图片paddingX，失败时为0
	/// \~english 
	/// \brief get the paddingX of image(GigE/Usb/PCIe Camera Only)
	/// \param [in] thiz this pointer
	/// \return the paddingX of image, return 0 when failed
	uint32_t(*getImagePaddingX)(struct GENICAM_Frame *thiz);

	/// \~chinese
	/// \brief 获取图片paddingY(仅对GigE/Usb/PCIe相机有效)
	/// \param [in] thiz 本接口指针
	/// \return 返回图片paddingY，失败时为0
	/// \~english 
	/// \brief get the paddingY of image(GigE/Usb/PCIe Camera Only)
	/// \param [in] thiz this pointer
...
/// \~chinese
/// \brief 相机对象接口类
/// \defgroup GENICAM_CLCamera cameraLink相机对象操作接口
/// @{
/// \~english
/// \brief camera object interface class
/// \defgroup GENICAM_CLCamera API for cameraLink Cameras Only
/// @{

// 32位系统下128字节
/// \~chinese													
/// \brief cameraLink相机特殊接口
/// \~english
/// \brief cameraLink camera specific object
typedef struct GENICAM_CLCamera
{
	/// \~chinese
	/// \brief 底层私有数据
	/// \~english
	...
	/// \brief add interface reference
	/// \param [in] thiz this pointer
	int32_t(*addRef)(struct GENICAM_CLCamera *thiz);
	...
	/// \brief release interface
	/// \param [in] thiz this pointer
	int32_t(*release)(struct GENICAM_CLCamera *thiz);
	...
	const char*  (*getVenderName)(struct GENICAM_CLCamera *thiz);
	...
	const char*  (*getModelName)(struct GENICAM_CLCamera *thiz);
	...
	const char*  (*getManufactureInfo)(struct GENICAM_CLCamera *thiz);
	...
	const char*  (*getVersion)(struct GENICAM_CLCamera *thiz);
	...
	const char*  (*getSerialNum)(struct GENICAM_CLCamera *thiz);
...
// 32位系统下128字节
/// \~chinese
/// \brief 创建CLCamera对象参数
/// \~english
/// \brief create cameraLink camera object's info object parameter
typedef struct GENICAM_CLCameraInfo
{
	GENICAM_Camera*  pCamera;	///< \~chinese 相机对象    \~english camera object
	uint32_t reserved[31];	    ///< \~chinese 保留        \~english reserved field
} GENICAM_CLCameraInfo;

/// \~chinese
/// 创建cameraLink设备对象
/// \param [in] pCLCameraInfo 创建CL设备对象的参数
/// \param [out] ppCLCamera CLCamera 对象
/// \retval < 0 创建失败
/// \retval 0   创建成功
/// \~english
/// create cameraLink camera object
/// \param [in]  pCLCameraInfo cameraLink camera info object
/// \param [out] ppCLCamera    cameraLink camera object
/// \retval < 0 create failed
/// \retval 0   create successfully
GENICAM_API int32_t CALL_METHOD GENICAM_createCLCamera(GENICAM_CLCameraInfo* pCLCameraInfo
	, GENICAM_CLCamera** ppCLCamera);

/// \~chinese
/// \brief 相机接口类
/// \defgroup GENICAM_CLInterface  连接cameraLink相机的接口
/// \~english
/// \brief camera interface 
/// \defgroup GENICAM_CLInterface API for cameraLink interface Only
/// @{
...
typedef enum GENICAM_EInterfaceType
{
...
}GENICAM_EInterfaceType;

typedef enum GENICAM_ETriggerMode
{
...
}GENICAM_ETriggerMode;

typedef enum GENICAM_EParamName
{
...
typedef struct GENICAM_EuresysInfo
{
...
typedef struct GENICAM_DalsaInfo
{
	uint16_t	index;				    ///< \~chinese 卡号					  \~english  Board index
...
typedef struct GENICAM_MatroxInfo
{
...
typedef struct GENICAM_InterfaceInfo
{
	union
	{
		GENICAM_EuresysInfo euresysInfo;	///< \~chinese Euresys采集卡信息		\~english  Euresys Information Object
		GENICAM_DalsaInfo dalsaInfo;		///< \~chinese Dalsa采集卡信息			\~english  Dalsa Information Object
		GENICAM_MatroxInfo matroxInfo;		///< \~chinese Matrox采集卡信息			\~english  Matrox Information Object			                                
	};
}GENICAM_InterfaceInfo;

/// \~chinese
/// \brief 相机接口类
/// \~english
/// \brief camera interface 
typedef struct GENICAM_CLInterface
{
	/// \~chinese
	/// \brief 底层私有数据
	/// \~english
	...
	/// \brief add interface reference
	/// \param [in] thiz this pointer
	int32_t(*addRef)(struct GENICAM_CLInterface *thiz);
	...
	/// \brief release interface
	/// \param [in] thiz this pointer
	int32_t(*release)(struct GENICAM_CLInterface *thiz);
	...
	const char * (*getDescription)(struct GENICAM_CLInterface *thiz);
	...
	const char * (*getVendorName)(struct GENICAM_CLInterface *thiz);
	...
	const char * (*getPortId)(struct GENICAM_CLInterface *thiz);
	...
	GENICAM_EInterfaceType (*getInterfaceType)(struct GENICAM_CLInterface *thiz);
	...
	/// \retval < 0 create failed
	/// \retval 0   create successfully
	int32_t (*setBoardConfig)(struct GENICAM_CLInterface *thiz, uint16_t index, uint16_t port, const char *pConfigFile);
	...
	/// \retval < 0 create failed
	/// \retval 0   create successfully
	int32_t (*getParamValue)(struct GENICAM_CLInterface *thiz, GENICAM_EParamName paramName, uint32_t *pVal);
	...
	/// \retval < 0 create failed
	/// \retval 0   create successfully
	int32_t (*setParamValue)(struct GENICAM_CLInterface *thiz, GENICAM_EParamName paramName, uint32_t val);
	...
	/// \retval < 0 create failed
	/// \retval 0   create successfully
	int32_t (*setInterfaceType)(struct GENICAM_CLInterface *thiz, GENICAM_EInterfaceType interfaceType);
	...
	/// \retval < 0 create failed
	/// \retval 0   create successfully
	int32_t (*setBoardConfigEx)(struct GENICAM_CLInterface *thiz, GENICAM_InterfaceInfo *interfaceInfo);
...
// 32位系统下128字节
/// \~chinese
/// \brief 创建cameraLink设备接口对象参数
/// \~english
/// \brief create cameraLink camera interface object parameter
typedef struct GENICAM_CLInterfaceInfo
{
	GENICAM_Camera*  pCamera;	///< \~chinese 相机对象    \~english camera object
	uint32_t reserved[31];	    ///< \~chinese 保留        \~english reserved field
} GENICAM_CLInterfaceInfo;

/// \~chinese
/// \brief 创建cameraLink设备接口对象
/// \param [in] pCLInterfaceInfo 创建cameraLink设备对象的参数
/// \param [out] ppCLInterface cameraLink Camera 所连接的接口对象
/// \retval < 0 创建失败
/// \retval 0   创建成功
/// \~english
/// create cameraLink camera interface object
/// \param [in] pCLInterfaceInfo parameter used to create cameraLink camera object
/// \param [out] ppCLInterface interface object which connect the cameraLink Camera
/// \retval < 0 create failed
/// \retval 0   create successfully
GENICAM_API int32_t CALL_METHOD GENICAM_createCLInterface(GENICAM_CLInterfaceInfo* pCLInterfaceInfo
	, GENICAM_CLInterface** ppCLInterface);
